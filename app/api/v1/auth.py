...
* **User token** (``typ=user``, ``sub=user.id``) — create, list, rename, and
  delete sessions. Also what ``/auth/logout`` accepts.
* **Session token** (``typ=session``, ``sub=session_id``, ``uid=owner``) — scoped
  to exactly one session. This is what conversation endpoints should depend on,
  so a leaked session token cannot enumerate the user's other sessions or mint
  new ones.

Async caveat: ``DatabaseService`` uses synchronous SQLAlchemy sessions inside
``async def``, so each call briefly blocks the event loop. Acceptable at current
traffic; the fix is an async engine across the whole service, not a partial
migration of these three dependencies.
"""
...
async def _reject_if_revoked(payload: dict[str, Any]) -> None:
    ...
    Costs one indexed primary-key read per authenticated request. Move to
    Valkey with a TTL matching the token expiry if that read becomes hot.
    """
    if await database_service.is_jti_revoked(payload["jti"]):
        raise HTTPException(status_code=401, detail="Token has been revoked", headers=_UNAUTHORIZED)
...
async def get_current_user(
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail=str(exc), headers=_UNAUTHORIZED) from exc
...
async def get_current_session(
    ...
    Re-checks that the session still belongs to the ``uid`` in the token, so a
    session that was deleted and its id reused, or reassigned, cannot be reached
    with the old token.

    Raises:
        HTTPException: 401 invalid, revoked, or wrong scope; 403 ownership
            mismatch; 404 the session no longer exists.
    """
    try:
...
async def _issue_login_tokens(user_id: int) -> tuple[Token, str]:
    ...
    await database_service.store_refresh_token(user_id, token_hash, refresh_expiry)
...
async def register_user(request: Request, user_data: UserCreate) -> UserResponse:
    ...
    try:
        ...
        # Hash the secret straight off the model. It is never sanitized: escaping
        # would change what gets hashed and break every password containing
        # characters like & or <.
    ...
    user = await database_service.create_user(email=email, password=hashed, username=username)
...
async def login(
    ...
    if user is None or not user.verify_password(password):
        ...
        raise HTTPException(
            status_code=401, detail="Incorrect email or password", headers=_UNAUTHORIZED
        )
...
async def refresh_access_token(request: Request, refresh_token: str = Form(...)) -> TokenResponse:
    """Rotate a refresh token for a fresh user token.

    Single-use: the presented token is revoked and a new one issued alongside the
    access token.
    """
    user_id = await database_service.consume_refresh_token(hash_refresh_token(refresh_token))
    if user_id is None:
        raise HTTPException(
...
async def logout(
    ...
    Deliberately not dependent on ``get_current_user``: logging out must work
    even for a user row that has since been deleted.
    """
    try:
        payload = decode_token(credentials.credentials, TokenType.USER)
    except InvalidTokenError as exc:
    ...
    await database_service.revoke_jti(
        payload["jti"], datetime.fromtimestamp(payload["exp"], tz=UTC)
    )
    revoked = await database_service.revoke_user_refresh_tokens(int(payload["sub"]))
    logger.info("user_logged_out", user_id=payload["sub"], refresh_tokens_revoked=revoked)
...
async def create_session(user: User = Depends(get_current_user)) -> SessionResponse:
    ...
    session = await database_service.create_session(session_id, user.id, username=user.username)
...
async def update_session_name(
    ...
    session = await database_service.update_session_name(session_id, sanitize_string(name))
    token = create_access_token(session_id, TokenType.SESSION, user_id=current_session.user_id)
    return SessionResponse(session_id=session_id, name=session.name, token=token)
