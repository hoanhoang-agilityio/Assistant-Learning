...
Tokens are held here and nowhere else. Not in the URL query string — proxies log
it and browsers keep it in history — and not on disk. The cost of that choice is
visible and deliberate: a full page reload starts a new Streamlit session and
the user signs in again. The conversations themselves are never lost, because
they live in Postgres and are re-listed on the next sign-in.
...
def sign_in(
    *,
    ...
    Args:
        email: The account's email address, shown in the sidebar footer.
        username: Optional display name.
        access_token: The user-scoped access token.
        expires_at: When that token expires, as returned by the API.
        refresh_token: The opaque refresh credential.
    """
...
def _expiry(auth: dict[str, Any]) -> datetime:
    """Parse the stored expiry.

    Args:
        auth: The stored auth record.

    Returns:
        The expiry as an aware UTC datetime. A value that cannot be parsed is
        treated as already expired, so the next call refreshes rather than
        sending a token the server may reject.
    """
    try:
...
def user_token(client: httpx.Client) -> str:
    ...
    Raises:
        RuntimeError: When called while signed out, which is a UI routing bug —
            ``app.py`` shows the sign-in gate instead.
        httpx.HTTPStatusError: When the refresh token has been spent or has
            expired. The caller signs the user out.
    """
...
def sync_conversations(client: httpx.Client) -> list[dict[str, Any]]:
    ...
    Newest first — a conversation list is read from the top, and the API returns
    creation order.
    ...
    Returns:
        The stored conversations, newest first.

    Raises:
        httpx.HTTPStatusError: When the user token is rejected.
    """
...
def active_conversation() -> dict[str, Any] | None:
    ...
    if session_id is None:
        return None
    return next(
        (item for item in st.session_state.conversations if item["session_id"] == session_id),
        None,
    )
...
def open_conversation(session_id: str, messages: list[dict[str, Any]]) -> None:
    ...
    Args:
        ...
        messages: Its stored messages, oldest first.
    """
...
def start_new_conversation() -> None:
    ...
    The conversation row is created when the first message is sent
    (``app/ui/app.py``), so clicking New chat repeatedly costs nothing and
...
def add_conversation(session: dict[str, Any]) -> None:
    ...
    Args:
        session: A ``SessionResponse`` body.
    """
...
def set_conversation_name(session_id: str, name: str) -> None:
    ...
    Args:
        session_id: The conversation that was renamed.
        name: Its new name.
    """
...
def remove_conversation(session_id: str) -> None:
    ...
    Args:
        session_id: The conversation that was deleted.
    """
    st.session_state.conversations = [
        item for item in st.session_state.conversations if item["session_id"] != session_id
    ]
