...
from app.ui.wording import ERROR_COPY, NEW_CONVERSATION_NAME, TAGLINE, conversation_title
...
def _render_conversation_actions(client: httpx.Client, conversation: dict[str, Any]) -> None:
    """Draw rename, clear and delete for the open conversation.

    Args:
        client: An open API client.
        conversation: The open conversation.
    """
    ...
    if columns[0].button("🧹 Clear", use_container_width=True, key=f"clear_{session_id}"):
        _clear(client, session_id)
    if columns[1].button("🗑 Delete", use_container_width=True, key=f"delete_{session_id}"):
...
def render_sidebar(client: httpx.Client) -> None:
    ...
    with st.sidebar:
        ...
        footer = st.columns(2)
        if footer[0].button("🔄 Refresh", use_container_width=True, key="refresh_conversations"):
            _refresh(client)
