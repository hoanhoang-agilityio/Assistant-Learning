...
def load_system_prompt(username: str | None = None, long_term_memory: str = "") -> str:
    """Render the shared system prompt.

    Args:
        username: Display name of the authenticated user, if known.
        long_term_memory: Memory retrieved for this turn. Empty when memory is
            unavailable or the user is anonymous.

    Returns:
        The formatted system prompt.
    """
    user_context = f"# User\n\nYou are talking to {username}.\n" if username else ""
    memory_block = f"# Memory\n\n{long_term_memory}" if long_term_memory else _NO_MEMORY
...
def load_qa_prompt(plan_context: str, long_term_memory: str = "") -> str:
    ...
    Args:
        ...
        long_term_memory: Facts retrieved for this turn at the root facade.
            Empty when memory is unavailable or the user is anonymous — the
            agent never searches for itself.
    ...
    return _QA_PROMPT_TEMPLATE.format(
        ...
        long_term_memory=long_term_memory or "Nothing recorded about this user yet.",
    )
