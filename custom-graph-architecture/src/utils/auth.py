...
1. A bearer token is never sanitized or HTML-escaped before decoding. A JWT is
   base64url; escaping is a no-op at best and corrupts the signature check at worst,
   while hiding real failures behind a generic 422.
2. Every decode asserts the expected ``typ``. Without that check a session token would
   be accepted wherever a user token is required and the privilege split between the two
   scopes would be decorative.
3. Every access token carries a ``jti`` so it can be denylisted at logout.
4. Session tokens carry ``uid`` so the session's owner can be re-verified against the
   token instead of trusted from the row alone.
"""
...
def hash_refresh_token(raw: str) -> str:
    ...
    Plain SHA-256 with no salt is correct here, unlike for passwords: the input is 48
    bytes of CSPRNG output, so there is no dictionary to attack and lookup by hash has
    to be deterministic.
    """
