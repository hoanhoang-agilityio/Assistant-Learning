...
### Routing rules
1. Prefer `qa` over `off_topic` for questions about training, pain, injuries, body composition, sleep, supplements, or nutrition.
2. Prefer `qa` over `coaching` when the user asks for general explanations, principles, or recommendations rather than a personalized plan.
3. Use `coaching` when the response requires the user's personal situation or profile.
...
3. Map the user's own words onto the closest listed value: "cutting" is FAT_LOSS, "bulking" is MUSCLE_GAIN, "desk job" is SEDENTARY, "gym 4x a week" is MODERATE.
...
6. When `<fields_in_focus>` names a field, read an otherwise-ambiguous message (e.g. a bare number) as answering that field rather than leaving it null.

## Injuries
Record an injury for every body part the user says is hurt, injured, recovering or painful, whether they are asking about it or reporting it.
...
## Preferences
Record what the user says they would *rather*, separately from what they are. A preference is honoured when nothing rules it out; it is never a reason to stop and ask.

- schedule: when or how they prefer to train, in their words. Not the number of days a week — that is `training_days_per_week` above.
...
Leave every one of them empty unless the user actually stated it. A movement they cannot do because it hurts is an injury, not a dislike.

## Revisions
If the user asks to change, correct, or update a field but does not restate a new value for it in this message (e.g. "my target weight is wrong", "update my profile"), add that field's name to `fields_to_revise`.
Never guess a replacement value for it — a value the user does state in the same message goes in the field itself, not in `fields_to_revise`.

{security}

## Output
Return the intent and the fields using the structured output schema, with null for anything unstated.
"""
...
TURN_PARSER_SYSTEM = _TURN_PARSER_SYSTEM_TEMPLATE.format(
    ...
    security=security_block("<user_message>"),
)
