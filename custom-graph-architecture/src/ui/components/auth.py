...
every other endpoint is behind a token: there is no anonymous mode to fall
back to. Registration and sign-in are two tabs rather than two pages, so a
failed sign-in does not lose the email that was already typed.

The password rules are imported from ``src.utils.sanitization`` rather than
...
from src.ui.wording import TAGLINE
from src.utils.sanitization import MIN_PASSWORD_LENGTH
...
def render_auth_gate(client: httpx.Client) -> None:
    ...
    st.markdown(
        ...
        '<p class="pt-welcome__title">🏋️ Coach AI</p>'
    ...
    if error:
        st.markdown(
            f'<div class="pt-form-error">⚠ {error}</div>', unsafe_allow_html=True
        )
