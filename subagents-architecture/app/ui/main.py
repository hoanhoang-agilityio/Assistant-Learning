"""Streamlit front end for the app API.

Run it from the project root::

    uv run streamlit run app/ui/main.py

It is a client of ``app.main``, not part of it: everything it knows comes from
``/api/v1``, so the two can be deployed separately and the UI cannot reach
around the API to the database or the graph.

Not named ``app.py``: Streamlit puts the entry script's own directory first on
``sys.path``, so a module called ``app`` here would shadow the ``app`` package
and every ``from app.ui import …`` below would fail. The same rule is why the
copy lives in ``wording.py`` rather than ``copy.py``, which would shadow the
standard library.
...
from app.ui.components.auth import render_auth_gate
...
from app.ui.components.welcome import render_welcome
from app.ui.wording import CHAT_PLACEHOLDER, ERROR_COPY
...
def inject_css() -> None:
    ...
    st.markdown(
        f"<style>{_STYLES_PATH.read_text(encoding='utf-8')}</style>", unsafe_allow_html=True
    )
...
def _open_most_recent(client: httpx.Client) -> None:
    ...
    Picking up where the user left off is the point of storing conversations at
    all. Skipped while they are composing a new chat, which is the one case
...
def _ensure_conversation(client: httpx.Client) -> str | None:
    ...
    Returns:
        The session id, or ``None`` when creation failed, in which case the
...
def main() -> None:
    ...
    with api_client.api_client() as client:
        ...
        if session_id is not None:
            # Echo before sending: the turn blocks for as long as the agent
